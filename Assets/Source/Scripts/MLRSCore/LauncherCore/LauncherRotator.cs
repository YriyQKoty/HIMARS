...
namespace Source.Scripts.MLRSCore.LauncherCore
{
    public class LauncherRotator : MonoBehaviour
    {
        ...
        [Header("Rotator params")][Space]
        [SerializeField] private Vector2 yAngleRange = new Vector2(-180, 180);
        ...
        public void Rotate(Vector3 angleVector)
        {
            ...
            _tweenHor = Math.Abs(angles.y - angleVector.y) < 2f ? 
                _horTransform.DOLocalRotate(new Vector3(0, angleVector.y, 0), rotTime * 0.05f).OnComplete(() =>
                {
            ...
            _tweenVert = Math.Abs(angles.x  - angleVector.x) < 2f ? 
                _vertTransform.DOLocalRotate(new Vector3(angleVector.x, 0, 0), rotTime * 0.05f).OnComplete(() =>
                {
